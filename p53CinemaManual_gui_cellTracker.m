...
u2 = uicontrol('Style','pushbutton','String','Custom Event','Units',get(f,'Units'),...
    'Position',[hwidth + hmargin_short + 0.5, hheight*(length(annotationNames)), hwidth, hheight],'parent',hbuttongroupTrackEvent,'HandleVisibility','on','Visible', 'on', ...
    'Callback',{@u2Pushbutton_Callback});
...
%
    ...
    function pushbuttonSaveAnnotations_Callback(~,~)
        ...
        centroidsCustom = master.obj_imageViewer.obj_cellTracker.centroidsCustom;
        annotationNames = master.additionalAnnotationNames;
        ...
        for i=1:length(centroidsTracks.singleCells)
            ...
            centroidsCustom.singleCells(i).point = centroidsCustom.singleCells(i).point / master.obj_imageViewer.imageResizeFactor;
            centroidsDeath.singleCells(i).point = centroidsDeath.singleCells(i).point / master.obj_imageViewer.imageResizeFactor;
        end
        
        uisave({'annotationNames', 'selectedGroup','selectedPosition','databaseFile','rawdatapath','centroidsTracks','centroidsDivisions','centroidsDeath','centroidsCustom'},...
        ...
        if(strcmp(etx, '.txt'))
            ...
            if(sum(validFields) < 2)
                ...
                for i=1:min(max(myCentroids.timepoint), length(master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells))
                    ...
                    if(any(strcmp(myCentroids.Properties.VariableNames, 'custom')))
                        subCustom = logical(myCentroids.custom(myCentroids.timepoint == i));
                        if(sum(subCustom) > 0)
                            master.obj_imageViewer.obj_cellTracker.centroidsCustom.singleCells(i).point(subIndex(subCustom),:) = subCentroids(subCustom,:) * master.obj_imageViewer.imageResizeFactor;
                        end
                    end
            ...
            for t=1:min(length(loadStruct.centroidsTracks.singleCells), length(master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells))
                ...
                if(any(strcmp(fieldnames(loadStruct), 'centroidsCustom')))
                    master.obj_imageViewer.obj_cellTracker.centroidsCustom.singleCells(t).point = loadStruct.centroidsCustom.singleCells(t).point;
                    master.obj_imageViewer.obj_cellTracker.centroidsCustom.singleCells(t).value = loadStruct.centroidsCustom.singleCells(t).value;
                end
            ...
            for i=1:length(master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells)
                ...
                master.obj_imageViewer.obj_cellTracker.centroidsCustom.singleCells(i).point = master.obj_imageViewer.obj_cellTracker.centroidsCustom.singleCells(i).point * master.obj_imageViewer.imageResizeFactor;
    ...
    function u2Pushbutton_Callback(source, eventdata)
        master.obj_imageViewer.obj_cellTracker.setCustomEvent;
        master.obj_imageViewer.setImage;
    end
